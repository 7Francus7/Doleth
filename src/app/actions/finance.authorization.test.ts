...
describe.skipIf(!hasDatabase)("autorización de las acciones financieras", () => {
  ...
  describe("sin sesión", () => {
    ...
    it("las acciones sin estado lanzan UnauthorizedError", async () => {
      ...
      await expect(
        voidMovementAction(formData({ id: alicia.transactionId, reason: "prueba sin sesión" })),
  ...
  it("no puede anular un movimiento ajeno", async () => {
    await expect(
      voidMovementAction(formData({ id: bruno.transactionId, reason: "intento de anulación cruzada" })),
    ).rejects.toThrow();
